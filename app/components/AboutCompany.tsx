        ...
        <div className="flex justify-center gap-1 md:gap-4 mb-10 ">
          {aboutTabs.map((tab) => (
            <button key={tab.id} onClick={() => handleAboutTabClick(tab.id)}
              className={`px-4 md:px-6 md:py-2 text-[14px] md:text-[16px]  ${
              ...
              <div className="relative h-[250px] md:h-[300px] flex items-center justify-center">
                <div className="flex justify-center items-center gap-2 md:gap-8 w-full">
                  {visibleCertificates.map((cert) => (
                    <div key={cert.id} className="w-[100px] md:w-[191px] flex items-center justify-center">
                      <div className="relative w-full h-[100px] md:h-[270px]">
                        <Image src={`/certificate${cert.id}.svg`} alt={`Сертификат ${cert.id}`} fill className="object-contain" />
                        </div>
                    </div>
                  ))}
                </div>
                
                <button onClick={handlePrevCertificates} disabled={!canScrollCertificatesPrev} className="absolute md:-left-7 -left-4  top-1/2 -translate-y-1/2 -translate-x-1/2 z-20 flex items-center justify-center" >
                  <img src="/ChevronLeft.svg" alt="ChevronLeft" className={`h-8 w-4 md:h-12 md:w-6 ${canScrollCertificatesPrev ? "text-black" : "text-[#8F8F8F]"}`}/>
                </button>
                
                <button onClick={handleNextCertificates} disabled={!canScrollCertificatesNext} className="absolute -right-4 md:-right-7 top-1/2 -translate-y-1/2 translate-x-1/2 z-20 flex items-center justify-center" >
                  <img src="/ChevronRight.svg" alt="ChevronRight" className={`h-8 w-4 md:h-12 md:w-6 ${canScrollCertificatesNext ? "text-black" : "text-[#8F8F8F]"}`} />
                </button>
              </div>
            </div>
          )}
