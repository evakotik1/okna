...
export async function ServeCached<T>(key: Key, ttl: number, fn: () => Promise<T>) {
    const k = key.filter((segment): segment is string => segment != null).join(":");
...
export async function InvalidateCached(key: Key) {
    ...
    const pattern = key.filter((segment): segment is string => segment != null).join(":").concat("*");
